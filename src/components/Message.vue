    ...
    <div v-for="(value, key, index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img class="icon" src="../assets/cross.png" @click="del(key)" alt v-if="patch && protile" />
...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios
              .delete("https://pure-dusk-80193.herokuapp.com/api/like", {
                data: {
        ...
        axios
          .post("https://pure-dusk-80193.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://pure-dusk-80193.herokuapp.com/api/shares/" +
